...
export default function Registro() {
  ...
  // crea el doc en /usuarios/{uid} si no existe
  const crearDocumentoUsuarioSiNoExiste = async (uid, dataExtra = {}) => {
    ...
    if (!snap.exists()) {
      await setDoc(ref, {
        ...
        nombre_completo: values.nombre_completo || dataExtra.displayName || "",
        email: values.email || dataExtra.email || "",
        rol: "USER",
        estado: "pendiente", // queda pendiente hasta que el admin apruebe
  ...
  const afterRegisterPending = async () => {
    // opcional: crear sesión de backend si ya tienes ese endpoint
    try {
      const token = await auth.currentUser.getIdToken(true);
      await axios
        .post("http://localhost:3000/auth/session", { idToken: token })
        .catch(() => {});
    } catch {}

    // cerramos sesión para que no pueda usar la cuenta sin aprobación
    try {
      await auth.signOut();
    } catch {}
    ...
    // lo mandamos al login de usuario
    navigate("/userlogin");
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await updateProfile(cred.user, {
        displayName: values.nombre_completo,
      });
        ...
        try {
          if (email) {
            ...
            if (methods?.includes("password")) {
              msg =
                "Este correo ya está registrado con correo y contraseña. Inicia sesión con tu clave.";
          ...
          >
            <div className="mgreg-password">
              <input
                ...
                onChange={(e) =>
                  setValues({ ...values, nombre_completo: e.target.value })
                }
